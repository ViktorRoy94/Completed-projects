...
int main ()
{
	...
	const int diag = 1 + _k*dt + 2*sigma*dt*(1/(dx*dx) + 1/(dy*dy) + 1/(dz*dz)); // элементы на диагонали матрицы. вводим с самого начала, чтобы потом не считать.
	...
	for(int i = 0; i < Sx-2; i++)				
		for(int j = 0; j < Sy-2; j++)
			...
			for(int y=0; y<Sy-2; y++)										//
				fprintf(filey,"%lf ", x1[24+y*(Sx-2)+24*(Sx-2)*(Sy-2)]);	// вывод в файл
	...
	for (int i = 0; i<(Sx-2)*(Sy-2)*(Sz-2); i++)							//z 
