...
namespace It_lab_graphic {
	...
	public:
		...
		FILE* fp_y;
		FILE* fp_z;
		int n,w1,h1,w2,h2,w3,h3; // w - width, h-height
...
#pragma endregion
	private: System::Void button1_Click(System::Object^  sender, System::EventArgs^  e) {
				fp_x=fopen("filexIDS.txt","r");
				fp_y=fopen("fileyIDS.txt","r"); 
				fp_z=fopen("filezIDS.txt","r"); 
				fscanf(fp_x,"%d %lf %lf \n",&n,&a,&b); // cчитываем кол-во точек и границы отрезка
				fscanf(fp_x,"%lf %lf",&y22,&y11); // считывает границы фун-ции f
	...
	private: System::Void pictureBox1_Paint(System::Object^  sender, System::Windows::Forms::PaintEventArgs^  e) {
				...
				if (count!=0) 
				{
					...
					for(int i=1;i<n-1;i++)
					{
						...
						y1=Math::Round(h1-mas[i]*h1/(b1-a1)/2);
						x2=Math::Round(((i+1)*w1)/n);
						y2=Math::Round(h1-mas[i+1]*h1/(b1-a1)/2);
